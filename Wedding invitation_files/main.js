...
function timer(clockID, exp_date) {
  ...
  const countDownDate = 1680408000000;
  ...
  const x = setInterval(function () {
    ...
    const the_days = days < 10 ? 0 + days : days;

    const the_hours = hours < 10 ? 0 + hours : hours;
    const the_minutes = minutes < 10 ? 0 + minutes : minutes;
    const the_seconds = seconds < 10 ? 0 + seconds : seconds;

    // Output the result in an element with id="demo"
    document.getElementById("days").innerText = the_days;
    document.getElementById("hours").innerText = the_hours;
    document.getElementById("mins").innerText = the_minutes;
    document.getElementById("secs").innerText = the_seconds;
  ...
  jQuery(".newsletter-field form").on("submit", function () {
    ...
    if (nlField.val() == "" || expEmail.test(nlField.val()) != true) {
      jQuery(this)
        .next()
        .html(
          '<div class="alert alert-success alert-dismissible mt-10"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> Please enter correct email address!</div>'
        );
      return false;
    } else {
      return;
...
jQuery(document).ready(function () {
  ...
  if (jQuery(".fat-nav").length > 0) {
    ...
    jQuery("ul li.menu-item-has-children > .toggle_button").on(
      "click",
      function () {
